...
# ══════════════════════════════════════════════════════════════════════════════
# المنطق:
#   • إذا requirements.txt موجود  → المكتبات مثبّتة مسبقاً → لا نثبّت شيئاً
#   • إذا requirements.txt غير موجود → نتحقق من كل مكتبة:
#       - موجودة  → لا نلمسها
#       - مفقودة  → نثبّتها فقط هي
...
def _auto_install():
    """
    المنطق الكامل للتثبيت التلقائي:

    الحالة 1 — requirements.txt موجود:
        يعني المستخدم ثبّت المكتبات يدوياً من قبل.
        لا نفعل شيئاً على الإطلاق — نتجاوز مباشرة.

    الحالة 2 — requirements.txt غير موجود:
        نتحقق من كل مكتبة في _PACKAGES:
        • موجودة  → تخطّاها (لا pip، لا شيء)
        • مفقودة  → ثبّتها فقط هي عبر pip مع نافذة تقدم
    """
    req_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "requirements.txt")

    # ── الحالة 1: requirements.txt موجود → توقف فوراً ──
    if os.path.exists(req_path):
        return  # المكتبات جاهزة — لا حاجة لأي شيء

    # ── الحالة 2: requirements.txt غير موجود → تحقق وثبّت المفقود فقط ──
    missing = [
        (imp, pip)
        for imp, pip in _PACKAGES.items()
        if not _pkg_installed(imp)  # فقط المكتبات غير الموجودة
    ]
    ...
    try:
        ...
        _tk.Label(_root, textvariable=_status, font=("Arial", 10),
                  fg="#c9d1d9", bg="#060d1a").pack(pady=5)

        _prog = _ttk.Progressbar(_root, length=380, mode="determinate")
        _prog.pack(pady=10, padx=40)

        _tk.Label(_root, text="يرجى الانتظار — هذا يحدث مرة واحدة فقط",
        ...
        _prog["value"] = 100
        _status.set(f"✓ تم تثبيت {len(missing)} مكتبة بنجاح!")
        _root.update()
        _t.sleep(1.2)
        _root.destroy()
...
# ── استيراد المكتبات بعد ضمان وجودها ────────────────────────────────────────
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                self.log_cb(
                    ...
                    "safe" if hasattr(self, "_log_has_safe") else "info"
                )
                self.notify_cb(
...
class MainApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # خط فاصل ملوّن أسفل الشريط العلوي
        sep = tk.Frame(self, bg="#00e5c3", height=2)
        sep.pack(fill="x")
        ...
        self.notebook.pack(fill="both", expand=True, padx=8, pady=(2, 5))

        style = ttk.Style()
        style.theme_use('clam')
        style.configure("TNotebook", background="#060d1a", borderwidth=0, tabmargins=[2, 4, 0, 0])
        ...
        style.map("TNotebook.Tab",
                  background=[("selected", "#00e5c3"), ("active", "#1a2f50")],
                  foreground=[("selected", "#060d1a"), ("active", "#c9e8ff")],
                  font=[("selected", ("Arial", 9, "bold"))])

        tabs = [
            ("dash_frame",      "📊 التحكم"),
            ("sell_frame",      "🛍 البيع"),
            ("products_frame",  "📦 المنتجات"),
            ("inv_frame",       "🧾 الفواتير"),
            ("manual_inv_frame","📝 فاتورة يدوية"),
            ("cashier_frame",   "🏪 الصناديق"),
            ("thread_vis_frame","🎨 مرئية الخيوط"),
            ("race_frame",      "⚡ Race Condition"),
            ("pc_frame",        "🔄 Producer-Consumer"),
            ("sched_frame",     "📅 CPU Scheduling"),
            ("perf_frame",      "📈 الأداء"),
            ("audit_frame",     "📋 Audit Log"),
            ("learn_frame",     "📚 تعلّم OS"),
            ("discount_frame",  "🏷 الخصومات"),
            ("delivery_frame",  "🚚 التوصيل"),
            ("cashdash_frame",  "🏦 Multi-Cashier D."),
            ("payments_frame",  "💳 المدفوعات"),
        ]
        for attr, label in tabs:
            fr = tk.Frame(self.notebook, bg="#060d1a")
        ...
        # تبويب إدارة الفواتير والمدفوعات — مضاف كـ PaymentManagerTab
    ...
    def _build_learn_tab(self):
        ...
        canvas_outer.create_window((0, 0), window=cards_frame, anchor="nw")
